...
import { Bell, Boxes, CalendarClock, ChevronDown, ChevronLeft, ChevronRight, CircleHelp, ClipboardList, Home, Menu, MoreHorizontal, PackagePlus, Search, Settings, ShieldCheck, UserRound, Wrench, X, ArrowUpRight, RefreshCw, Pencil, Trash2, Users } from 'lucide-react';
...
export function Sidebar() {
  ...
  const nav = [{ href: '/workspace', label: 'Overview', icon: Home }, { href: '/inventory', label: 'Inventory', icon: Boxes }, { href: '/maintenance', label: 'Maintenance', icon: Wrench }, { href: '/directory', label: 'Directory', icon: Users }];
  return <aside className="sidebar">
    ...
    <nav className="side-nav"><div className="nav-section">Workspace</div>{nav.map(({ href, label, icon: Icon }) => <Link key={href} href={href} data-testid={`link-${label.toLowerCase()}`} className={`nav-item ${location === href ? 'active' : ''}`}><Icon size={18} /><span className="nav-label">{label}</span>{label === 'Inventory' && <span className="nav-count">20</span>}</Link>)}</nav>
    <div className="sidebar-foot"><div className="health-row"><span className="health-dot" /> All systems operational</div><Link href="/settings" className={`nav-item ${location === '/settings' ? 'active' : ''}`} data-testid="link-settings"><Settings size={18} /><span className="nav-label">Settings</span></Link><button className="profile profile-button" onClick={() => void signOut({ redirectUrl: '/' })}><span className="avatar">{(user?.firstName?.[0] ?? user?.emailAddresses[0]?.emailAddress[0] ?? "U").toUpperCase()}</span><span className="profile-copy"><b>{user?.fullName ?? user?.emailAddresses[0]?.emailAddress ?? "Signed-in operator"}</b><small>Administrator · Sign out</small></span><MoreHorizontal size={17} /></button></div>
  </aside>;
}
