...
import { Bell, Boxes, CalendarClock, ChevronDown, ChevronLeft, ChevronRight, CircleHelp, ClipboardList, Home, Menu, MoreHorizontal, PackagePlus, Search, Settings, ShieldCheck, UserRound, Wrench, X, ArrowUpRight, RefreshCw } from 'lucide-react';
...
export function Sidebar() {
  ...
  const nav = [{ href: '/', label: 'Overview', icon: Home }, { href: '/inventory', label: 'Inventory', icon: Boxes }, { href: '/maintenance', label: 'Maintenance', icon: Wrench }];
  return <aside className="sidebar">
    ...
    <nav className="side-nav"><div className="nav-section">Workspace</div>{nav.map(({ href, label, icon: Icon }) => <Link key={href} href={href} data-testid={`link-${label.toLowerCase()}`} className={`nav-item ${location === href ? 'active' : ''}`}><Icon size={18} /><span className="nav-label">{label}</span>{label === 'Inventory' && <span className="nav-count">486</span>}</Link>)}</nav>
    <div className="sidebar-foot"><div className="health-row"><span className="health-dot" /> All systems operational</div><Link href="/settings" className={`nav-item ${location === '/settings' ? 'active' : ''}`} data-testid="link-settings"><Settings size={18} /><span className="nav-label">Settings</span></Link><div className="profile"><span className="avatar">MC</span><span className="profile-copy"><b>Marin Chen</b><small>Administrator</small></span><MoreHorizontal size={17} /></div></div>
  </aside>;
}
...
export function MaintenanceList({ items, limit }: { items: MaintenanceItem[]; limit?: number }) { const shown = limit ? items.slice(0, limit) : items; return <div className="maintenance-list">{shown.map((item) => <div className="maintenance-item" key={item.id} data-testid={`maintenance-${item.id}`}><div className="date-block"><b>{new Date(item.scheduledAt).toLocaleDateString('en-US', { day: '2-digit' })}</b><span>{new Date(item.scheduledAt).toLocaleDateString('en-US', { month: 'short' })}</span></div><div className="maintenance-main"><div><b className="mono">{item.assetTag}</b><span className="item-category">{item.category}</span></div><p>{item.technician}</p></div><span className={`priority priority-${item.priority}`}>{item.priority}</span><StatusPill status={item.status} /></div>)}</div>; }